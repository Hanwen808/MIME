...
// adjust mime's parameters
// Note: there is only one hash function, so it same like bitmap

/*
 * N: the number of distinct elements in current period
 * prob: the overall sampling rate of MIME
 * */
int calculateBloomFilterParameters(int mN, double prob) {
    return ceil(-1.0 * mN / log(prob));
}

/*
 * N: the number of distinct flow elements during old period
 * bits: the bits of bloom filter
 * */
double calculateSamplingrate(int bits, int N) {
    return exp(- 1.0 * N / bits);
}
...
int main() {
    ...
    string outputFilePathDC = "../data/spreads_mime_dc.txt";
    string outputFilePathDPC = "../data/spreads_mime_dpc.txt";
    string outputFilePathSC = "../data/spreads_mime_sc.txt";
