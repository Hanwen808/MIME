...
void processPackets(Sketch* sktdc, Sketch* sktdpc, Sketch* sktsc, vector<pair<pair<uint32_t, uint32_t>, uint32_t>>& dataset) {
    clock_t start = clock(); // The start time of processing packet stream
    for (int i = 0; i < dataset.size(); i++) {
        sktdc->update(dataset[i].first.first, dataset[i].first.second, dataset[i].second);
        sktdpc->update(dataset[i].first.first, dataset[i].second, dataset[i].first.second);
        sktsc->update(dataset[i].first.second, dataset[i].first.first, dataset[i].second);
    }
...
void saveResults(string outputFilePath, Sketch* skt, unordered_map<uint32_t , unordered_set<uint32_t>>& realFlowInfo, int taskId) {
    ...
    //char key1[KEY_LEN] = "82.96.243.186";
    ...
    while (fout.is_open() && iter != realFlowInfo.end()) {
        ...
        /*if (memcmp(key1, iter->first, KEY_LEN) == 0) {
            cout << realSpread <<  " " << estimatedSpread << endl;
        }*/
...
int main() {
    ...
    cout << endl;
    unsigned int regNum = (100 * 1024 * 8) / 5;
    unsigned int v = 64;
    cout << "Total space is " << 3.0 * (regNum * 5) / 8 / 1024 << "KB" << endl;
    vHLL* sktdc = new vHLL(regNum, v);
    vHLL* sktdpc = new vHLL(regNum, v);
    vHLL* sktsc = new vHLL(regNum, v);
    cout << endl;
    cout << "Start processing..." << endl;
    processPackets(sktdc, sktdpc, sktsc, dataset);
    //save the result in files
    cout << endl;
    cout << "save the result in spreads.txt ..." << endl;
    string outputFilePathDC = "./result/spreads_vhll_dc.txt";
    string outputFilePathDPC = "./result/spreads_vhll_dpc.txt";
    string outputFilePathSC = "./result/spreads_vhll_sc.txt";
    ...
    sktdpc->update_param();
    sktsc->update_param();
    saveResults(outputFilePathDC, sktdc, realDCFlowInfo, 0);
    cout << "DC has been saved." << endl;
    saveResults(outputFilePathDPC, sktdpc, realDPCFlowInfo, 1);
    cout <<"DPC has been saved." << endl;
    saveResults(outputFilePathSC, sktsc, realSCFlowInfo, 2);
